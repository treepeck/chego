...
var (
	...
	// Generated by the GenMagicNumber function. See commit c0cfb607e20a1e469d2fc94b26146645bc1fc9a1
	// for details.
...
func IsSquareUnderAttack(bitboards [15]uint64, square int, c Color) bool {
	occupancy := bitboards[14]
	// Color offset.
	o := 6 * c
	// If attacked by pawns.
	return (pawnAttacks[c^1][square]&bitboards[o+PieceWPawn] != 0) ||
		// If attacked by knights.
		(knightAttacks[square]&bitboards[o+PieceWKnight] != 0) ||
		// If attacked by bishops.
		(lookupBishopAttacks(square, occupancy)&bitboards[o+PieceWBishop] != 0) ||
		// If attacked by rooks.
		(lookupRookAttacks(square, occupancy)&bitboards[o+PieceWRook] != 0) ||
		// If attacked by queens.
		(lookupQueenAttacks(square, occupancy)&bitboards[o+PieceWQueen] != 0) ||
		// If attacked by king.
		(kingAttacks[square]&bitboards[o+PieceWKing] != 0)
}
...
func GenLegalMoves(p Position, l *MoveList) {
	...
	for i := 0; i < int(pseudoLegal.LastMoveIndex); i++ {
		...
		king := BitScan(p.Bitboards[PieceWKing+(6*p.ActiveColor)])
		if !IsSquareUnderAttack(p.Bitboards, king, 1^p.ActiveColor) {
...
func genPawnMoves(pawn int, enemies, occupancy, epTarget uint64, c Color,
	...
	for attacks > 0 {
		atackSq := PopLSB(&attacks)

		if 1<<atackSq&promoRank != 0 {
			l.Push(NewMove(atackSq, pawn, MovePromotion))
		} else if 1<<atackSq&epTarget != 0 {
			l.Push(NewMove(atackSq, pawn, MoveEnPassant))
		} else {
			l.Push(NewMove(atackSq, pawn, MoveNormal))
		}
	}
}
...
func genNormalPieceMoves(square int, piece Piece, allies, occupancy uint64,
	l *MoveList) {
	var dests uint64

	switch piece {
	case PieceWKnight, PieceBKnight:
		dests = knightAttacks[square]

	case PieceWBishop, PieceBBishop:
		dests = lookupBishopAttacks(square, occupancy)

	case PieceWRook, PieceBRook:
		dests = lookupRookAttacks(square, occupancy)

	case PieceWQueen, PieceBQueen:
		dests = lookupQueenAttacks(square, occupancy)
	}
	...
	for dests > 0 {
		...
		bb := uint64(1 << dst)
		if bb&allies == 0 {
			l.Push(NewMove(dst, square, MoveNormal))
		}
	}
}
...
func genKingMoves(p Position, king int, l *MoveList) {
	enemies := p.Bitboards[12+(1^p.ActiveColor)]
	occupancy := p.Bitboards[14]
	// Lookup normal moves.
	attacks := kingAttacks[king]
	for attacks > 0 {
		square := PopLSB(&attacks)
		if 1<<square&enemies != 0 || 1<<square&occupancy == 0 {
			l.Push(NewMove(square, king, MoveNormal))
		}
	}
...
func genAttacks(bitboards [15]uint64, c Color) uint64 {
	...
	attacked := genPawnAttacks(bitboards[PieceWPawn+6*c], c)
	attacked |= genKnightAttacks(bitboards[PieceWKnight+6*c])
	attacked |= genKingAttacks(bitboards[PieceWKing+6*c])

	for i := PieceWBishop + 6*c; i <= PieceWKing+6*c; i++ {
		for bitboards[i] > 0 {
